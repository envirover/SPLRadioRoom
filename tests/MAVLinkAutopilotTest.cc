...
//constexpr struct timespec heart_interval[] = { { 0, 10000000L } }; // 10 milliseconds
...
int main(int argc, char** argv)
{
    ...
    cout << "MAVLinkAutopilot.get_system_id() returned " << (int)autopilot.get_system_id() << endl;

    request_data_streams(autopilot, DATA_STREAM_RATE);

    high_resolution_clock::time_point start_time     = high_resolution_clock::now();
    high_resolution_clock::time_point heartbeat_time = high_resolution_clock::now();

    int          count  = 0;
    const double period = 10.0;

    while (duration_cast<milliseconds>(high_resolution_clock::now() - start_time).count() / 1000.0 <= period) {
        mavlink_message_t msg;
        if (autopilot.receive_message(msg)) {
            cout << "Message reseived. msgid = " << (int)msg.msgid << endl;
            count++;
        }

        if (duration_cast<milliseconds>(high_resolution_clock::now() - heartbeat_time).count() / 1000.0 >= 1.0) {
